    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <!-- 小图列表 -->
          <ImageList :imageList="info.skuInfo.skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          <div class="goodsDetail">
            <h3 class="InfoName">
              {{ info.skuInfo.skuName }}
            </h3>
            <p class="news">
              {{ info.skuInfo.skuDesc }}
            </p>
            <div class="priceArea">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="spuSaleAttr in info.spuSaleAttrList"
                :key="spuSaleAttr.id"
              >
                ...
                <dd
                  ...
                  v-for="s2 in spuSaleAttr.spuSaleAttrValueList"
                  :key="s2.id"
                  @click="changeAttr(spuSaleAttr.spuSaleAttrValueList, s2.id)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                />
                <a href="javascript:" class="plus" @click="changeGoodsNum('in')"
                  >+</a
                >
                <a href="javascript:" class="mins" @click="changeGoodsNum('de')"
                  >-</a
                >
              </div>
              <div class="add">
                <a href="javascript:" @click="handlerAddCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
import { reqGoodsInfo, reqAddGoods2Cart } from "@/api";
import { goodsNumReg } from "@/utils/reg.js";
...
export default {
  ...
  mounted() {
    //联系服务员
    //组件一挂载，就调用getGoodsInfo去请求商品的详细信息
    this.getGoodsInfo();
  },
  methods: {
    //获取商品详细
    ...
    changeGoodsNum(type, e) {
      if ((type = "input")) {
        ...
        if (result) {
          ...
          // console.log('你输入的不合法,超出200，我帮你重置200')
    ...
    async handlerAddCart() {
      ...
      if (result.code === 200) {
        ...
        const { goodsNum } = this;
        //提前准备一个数组，用于存储用户选择的具体信息
        const arr = [];
        //遍历所有商品的属性信息，收集用户所选择的
        this.info.spuSaleAttrList.forEach((spuSaleAttr) => {
          const result = spuSaleAttr.spuSaleAttrValueList.find(
            (s2) => s2.isChecked == 1
          );
          arr.push(result.saleAttrName + ":" + result.saleAttrValueName)
        });
        const goodsInfo = { skuName, price, goodsNum, arr ,skuDefaultImg}
        //将收集好的信息存入sessionStorage
