<template>
  <div class="trade-container">
    ...
    <div class="sub clearFix">
      <a class="subBtn" @click="handleSubmitOrder">提交订单</a>
    </div>
  </div>
</template>

<script>
import { reqAddressList, reqTradeInfo, reqSubmitOrder } from "@/api";
export default {
  name: "Trade",
  data() {
    return {
      AddressList: [], //收货地址
      TradeInfo: {}, //交易信息（商品清单，数量，）
      message: "", //留言
    };
  },
  computed: {
  ...
  methods: {
    ...
    //切换收货地址的回调
    handleChenge(id) {
      this.addressList.forEach((adress) => {
        if (adress.id === id) {
          address.isDefault = "1";
        } else {
          address.isDefault = "0";
